<nav class="fixed top-4 sm:top-6 inset-x-0 z-50 px-4 max-w-5xl mx-auto">
    <div class="relative rounded-full bg-slate-900/75 backdrop-blur-xl border border-white/20 shadow-[0_10px_40px_rgba(0,0,0,0.6)] px-5 sm:px-8 py-2.5 flex items-center justify-between gap-4 transition-all duration-300">
        
        <!-- Logo Brand -->
        <a href="{{ route('home') }}" class="flex items-center gap-2.5 group">
            <img src="{{ asset('images/himsi.png') }}" alt="Logo HIMSI" class="w-8 h-8 rounded-lg p-0.5 bg-white shadow-md group-hover:scale-110 transition-transform">
            <div class="flex flex-col">
                <span class="text-sm font-black text-white tracking-wider group-hover:text-amber-400 transition-colors">
                    HIMSI <span class="text-amber-400">OPREC</span>
                </span>
                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest -mt-0.5">UBSI 2026</span>
            </div>
        </a>
