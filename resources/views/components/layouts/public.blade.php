...
<body class="bg-[#f9f9fc] text-[#1a1c1e] antialiased flex flex-col min-h-screen">
    ...
    <div x-data="{ 
         ...
         class="fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-[9999] flex flex-col items-end gap-3 sm:gap-4">
        
        <!-- Scroll to Top Button (Desktop Only, Ukuran Presisi h-14 w-14 sama dengan WhatsApp) -->
        <button @click="window.scrollTo({ top: 0, behavior: 'smooth' })"
                ...
                class="group relative hidden sm:flex h-14 w-14 items-center justify-center rounded-full bg-[#001b79] text-white shadow-lg shadow-blue-950/25 transition-all duration-300 hover:bg-[#000c46] hover:scale-105">
            
            <!-- SVG Circular Scroll Progress Ring surrounding the arrow -->
            <svg class="absolute inset-0 h-full w-full -rotate-90 transform p-0.5" viewBox="0 0 64 64">
            ...
            <svg class="h-6 w-6 relative z-10 text-white transition-transform duration-300 group-hover:-translate-y-0.5" 
                 fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M5 10l7-7m0 0l7 7m-7-7v18"/>
            </svg>
            
            <!-- Tooltip Hover -->
            <span class="absolute right-16 whitespace-nowrap rounded-lg bg-[#000c46] px-2.5 py-1 text-[11px] font-semibold text-white shadow-md opacity-0 pointer-events-none transition-all duration-300 group-hover:opacity-100 group-hover:pointer-events-auto">
                Kembali ke Atas
            </span>
        </button>

        <!-- Official WhatsApp Floating Button (Ukuran Presisi h-14 w-14 sama dengan Scroll-to-Top) -->
        <a href="{{ $waUrl }}" target="_blank" rel="noopener" 
           title="Hubungi Kami via WhatsApp"
           class="relative h-14 w-14 rounded-full bg-[#25D366] hover:bg-[#20ba5a] text-white flex items-center justify-center shadow-[0_4px_24px_rgba(37,211,102,0.4)] hover:scale-105 transition-all duration-300 group">
            <!-- Pulsing outer ring animation -->
            <span class="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-30"></span>
            
            <!-- WhatsApp Official Icon -->
            <svg class="w-7 h-7 relative z-10 text-white fill-current transform group-hover:rotate-12 transition-transform duration-300" viewBox="0 0 24 24">
