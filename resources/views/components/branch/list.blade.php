...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-10">
    ...
    <div class="rounded-2xl p-6 bg-white border border-[#c5c5d4]/60 shadow-[0_4px_20px_rgba(0,27,121,0.04)] space-y-4">
        <form method="GET" action="{{ route('branch.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="md:col-span-6 relative">
                ...
                <svg class="absolute left-3.5 top-3.5 h-4 w-4 text-[#0453cd]" fill="none" viewBox="0 0 24 24"
            ...
            <div class="md:col-span-3">
                <select name="type" onchange="this.form.submit()"
                    class="w-full rounded-xl border border-[#c5c5d4]/60 bg-[#f0f4ff]/50 px-4 py-3 text-sm text-[#000c46] focus:border-[#0453cd] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0453cd]/20 transition-all cursor-pointer">
                    <option value="">Semua Tingkat (DPP & DPC)</option>
                    <option value="dpp" {{ $currentType === 'dpp' ? 'selected' : '' }}>Dewan Pimpinan Pusat (DPP)
                    </option>
                    <option value="dpc" {{ $currentType === 'dpc' ? 'selected' : '' }}>Dewan Pimpinan Cabang (DPC)
                    </option>
                </select>
            </div>
            ...
            <div class="md:col-span-3">
                <select name="sektor" onchange="this.form.submit()"
                    ...
                    @foreach ($sektors as $sektor)
                        <option value="{{ $sektor }}" {{ $currentSektor === $sektor ? 'selected' : '' }}>
                            {{ $sektor }}</option>
                    @endforeach
                </select>
            </div>
    ...
    @if (count($branches) > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            @foreach ($branches as $branch)
                <div
                    class="group rounded-2xl bg-white border border-[#c5c5d4]/60 shadow-[0_4px_16px_rgba(0,27,121,0.04)] hover:shadow-[0_12px_32px_rgba(0,27,121,0.12)] hover:border-[#0453cd]/40 transition-all duration-300 overflow-hidden flex flex-col justify-between">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="h-64 sm:h-72 lg:h-80 w-full overflow-hidden relative bg-[#f0f4ff]/70">
                            ...
                            <div class="absolute top-4 right-4 flex items-center gap-1.5 z-10">
                                @if ($branch['is_dpp'])
                                    <span
                                        class="rounded-full bg-[#000c46] px-3 py-1 text-xs font-bold text-white uppercase tracking-wider shadow-xs">DPP</span>
                                @endif
                                <span
                        ...
                        <div class="p-6 space-y-3">
                            ...
                            @if (isset($branch['description']))
                                <p class="text-sm text-[#454652] line-clamp-3 leading-relaxed">
                                    {{ \Illuminate\Support\Str::limit(strip_tags($branch['description']), 150) }}
                                </p>
                            @endif
                        </div>
                    </div>
    ...
    @else
        <x-common.empty-state title="Cabang Tidak Ditemukan"
            message="Tidak ada data cabang yang sesuai dengan pencarian/filter Anda." />
    @endif
