...
<section class="relative min-h-screen flex items-center overflow-hidden isolate pt-20 pb-12" x-data="{
    ...
    <div class="absolute inset-0 -z-20 pointer-events-none">
        <video x-ref="heroVideo" id="heroVideoEl"
            class="h-full w-full object-cover opacity-90 scale-105 pointer-events-none" autoplay muted="muted" loop
            playsinline webkit-playsinline preload="auto" tabindex="-1"
            controlslist="nodownload nofullscreen noremoteplayback" x-init="$nextTick(() => {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            ['click', 'touchstart', 'scroll', 'mousemove'].forEach(function(evt) {
                window.addEventListener(evt, playHeroVideo, {
                    once: true
                });
            });
        });
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <div class="max-w-3xl py-12 sm:py-20 lg:py-24 space-y-6 sm:space-y-7">
            ...
            <div class="flex flex-wrap items-center gap-3 pt-2">
                <span
                    class="inline-flex items-center gap-2 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/40 px-4 py-1.5 text-xs font-extrabold backdrop-blur-md shadow-sm">
            ...
            <div class="space-y-2">
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight text-white leading-[1.15]">
                    ...
                    <span class="text-amber-400 block my-1 min-h-[1.25em]">
                        <span x-text="currentText">Sistem Informasi</span><span
                            class="animate-pulse text-amber-400">|</span>
                    </span>
                </h1>
            </div>
            ...
            <p
                class="text-base sm:text-lg lg:text-xl text-slate-200 leading-relaxed font-medium max-w-2xl drop-shadow-sm">
            ...
            <!-- Key Feature Highlights Badges (3 Poin Utama) -->
            <div class="flex flex-wrap items-center gap-3 text-xs sm:text-sm font-semibold text-slate-200 pt-1">
                <span
                    class="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-2 border border-white/15 backdrop-blur-md shadow-sm">
